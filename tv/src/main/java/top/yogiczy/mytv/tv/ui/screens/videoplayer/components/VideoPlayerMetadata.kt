...
fun VideoPlayerMetadata(
    ...
    CompositionLocalProvider(
        ...
        Column(
            ...
            Column {
                Text("視頻", style = MaterialTheme.typography.titleMedium)
                Column(modifier = Modifier.padding(start = 10.dp)) {
                    Text("編碼: ${metadata.videoMimeType}")
                    Text("解碼器: ${metadata.videoDecoder}")
                    Text("分辨率: ${metadata.videoWidth}x${metadata.videoHeight}")
                    Text("色彩: ${metadata.videoColor}")
                    Text("幀率: ${metadata.videoFrameRate}")
                    Text("比特率: ${metadata.videoBitrate / 1024} kbps")
                }
            }

            Column {
                Text("音頻", style = MaterialTheme.typography.titleMedium)
                Column(modifier = Modifier.padding(start = 10.dp)) {
                    Text("編碼: ${metadata.audioMimeType}")
                    Text("解碼器: ${metadata.audioDecoder}")
                    Text("聲道數: ${metadata.audioChannels}")
                    Text("採樣率: ${metadata.audioSampleRate} Hz")
                }
            }
        }
    }
